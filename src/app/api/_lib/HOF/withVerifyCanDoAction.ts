...
const getPermissionsForRole = cache(
  async (roleId: string, data: TDataCanDoAction) => {
...
export const withVerifyCanDoAction =
  <T extends THofContext>(
    data: TDataCanDoAction,
    callback: (req: NextRequest, prevCtx: T) => Promise<Response>
  ) =>
...
const getPermissionsForRoles = cache(
  async (roleId: string, datas: TDataCanDoActions[]) => {
    return prisma.role.findFirst({
      where: {
        id: roleId,
        permissions: {
...
export const withVerifyCanDoActions =
  <T extends THofContext>(
