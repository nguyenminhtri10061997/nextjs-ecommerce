...
export default function User() {
  ...
  const { pagination, orderQuery, setPagination, setOrderQuery } =
    usePaginationAndSort<keyof Option>({
      defaultOrder: {
        orderKey: "createdAt",
        orderType: "desc",
      },
  ...
  const query = useGetOptionListQuery({
    orderQuery: {
  ...
  const columns: TColumn<Option>[] = useMemo(
    () => [
      {
        key: "name",
        header: "Name",
      ...
      },
      {
        ...
        render: (_, row) => {
          return (
            <Stack direction="row">
              <AppLinkWithLoading href={`option/${row.id}`}>
                <IconButton>
  ...
  return (
    ...
    >
      <Box sx={{ flexShrink: 0 }}>
        <Typography variant="h4">Option Page</Typography>
      </Box>
      <Box
        sx={{
          flexGrow: 1,
        ...
        <AppTable
          ...
          actions={
            <Stack direction="row" alignItems="center" spacing={2}>
              <AppLinkWithLoading href="option/create">
