...
import { productTagKeys, deleteProductTags } from "@/lib/reactQuery/product-tag";
...
export default function usePage(props: TProps) {
  ...
  const mutation = useMutation({
    mutationFn: deleteProductTags,
    onSuccess: async (_, variable) => {
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: productTagKeys.lists() }),
        queryClient.invalidateQueries({ queryKey: productTagKeys.details() }),
      ]);
