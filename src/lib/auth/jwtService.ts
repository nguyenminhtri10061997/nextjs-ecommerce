...
export class JwtService {
    ...
    static async generateRefreshToken(payload: TDataToken) {
        return jwt.sign(payload, this.jwtSecret, { expiresIn: this.jwtRefreshExpiresIn });
    }

    static verifyToken(token: string, options?: VerifyOptions): TDataToken {
    ...
    static generateAccessTokenAndExpired = async (account: Account, sessionId: string) => {
